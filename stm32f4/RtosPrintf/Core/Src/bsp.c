 ...
 * 引脚、时钟等板级接线见 stm32f4xx_hal_msp.c 的 HAL_UART_MspInit。 */
static void MX_USART1_UART_Init(void)
{
...
{
    volatile uint32_t timeout = 1000000UL; /* 轮询上限,约数毫秒 */

    while ((huart1.Instance->SR & USART_SR_TXE) == 0U)
    {
        if (--timeout == 0U)
        {
            return; /* 串口不可用:丢字节,不卡死 */
        }
    }
    huart1.Instance->DR = ch; /* 写 DR 同时清除 TXE */
}
