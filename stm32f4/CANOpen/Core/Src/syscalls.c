...
/* printf 重定向目标:USART1(见 bsp.c 的 MX_USART1_Init) */
#include "bsp.h"
 ...
 * __io_putchar(). 现在把每个字符经 USART1 轮询发送,printf 即可在串口输出。 */
 ...
 * "_end" symbol up to the reserved MSP stack (see STM32F407XX_FLASH.ld).
 * If the heap would grow into the stack region, return ENOMEM. */
